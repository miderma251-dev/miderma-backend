        ...
        <section className="py-16 md:py-24 bg-white">
            <div className="max-w-3xl mx-auto px-4 sm:px-6">
                ...
                <div className="text-center mb-12">
                    <h2 className="text-3xl md:text-5xl font-bold text-[#291840] mb-4 font-serif">Preguntas Frecuentes</h2>
                    <p className="text-[#9A92A6]">Resolvemos tus dudas antes de tu consulta.</p>
                </div>
                ...
                <div className="space-y-4">
                    {faqs.map((faq, index) => (
                        <div 
                            key={index} 
                            className="border border-[#F2F2F2] rounded-2xl overflow-hidden transition-all duration-300"
                        >
                            <button 
                                className="w-full px-6 py-5 text-left flex justify-between items-center bg-[#F2F2F2]/50 hover:bg-[#FDF6F4] transition-colors"
                                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                            >
                                <span className="font-bold text-[#291840] text-sm md:text-base pr-4">{faq.pregunta}</span>
                                <span className={`transform transition-transform duration-300 ${openIndex === index ? 'rotate-180' : ''}`}>
                                    <svg className="w-5 h-5 text-[#F2BDC7]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            >
                                <div className="p-6 bg-white text-sm text-[#615573] leading-relaxed border-t border-[#F2F2F2]">
                                    {faq.respuesta}
                                </div>
                            </div>
                        </div>
